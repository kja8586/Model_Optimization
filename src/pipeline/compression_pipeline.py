...
def run_pipeline():
  ...
  log_strategy("STRATEGY 1 - LARGE BASE MODEL", large_acc, base_size, base_gzip, base_params, base_filters)
  ...
  log_strategy("STRATEGY 2 - STRUCTURED PRUNING", structured_acc, structured_size, structured_gzip, structured_params, structured_filters)
  ...
  log_strategy("STRATEGY 3 - UNSTRUCTURED PRUNING", unstructured_acc, unstructured_size, unstructured_gzip, unstructured_params, unstructured_filters)

  # Step 4: Clustering
  clustered_model = apply_clustering(unstructured_model, num_clusters=config["clusters"])
  clustered_hist = train(clustered_model, x_train, y_train, epochs=config["epochs"], batch_size=config["batch_size"])
  ...
  log_strategy("STRATEGY 4 - CLUSTERING", clustered_acc, clustered_size, clustered_gzip, clustered_params, clustered_filters)
  ...
  log_strategy("STRATEGY 5 - QUANTIZATION", quantized_acc, quantized_size, quantized_gzip, quantized_params, quantized_filters) 

  # Step 6: Convert to TFLite int8
  tflite_model = convert_to_tflite_int8(quantized_model)
  ...
  log_strategy("STRATEGY 6 - TFLITE INT8", tflite_acc, tflite_size, tflite_gzip, quantized_params, quantized_filters)
